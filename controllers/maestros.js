  ...
  maestros: function (req, res) {
    Maestros.find({}).exec((err, maestros) => {
      ...
      if (!maestros)
        return res
          .status(200)
          .json({ status: 200, mensaje: "No hay maestros por listar" });
  ...
  crear_maestro: function (req, res) {
    ...
    Maestros.findOne({ n_cuenta: user_info.n_cuenta }).exec((err, maestro) => {
      ...
      if (maestro)
        return res
          .status(200)
          .json({ status: 200, mensaje: "El numero de cuenta ya existe" });
      ...
      maestros_model.save((err, maestroStored) => {
        ...
        if (!maestroStored)
          return res
            .status(200)
            .json({ status: 200, mensaje: "No logró almacenar el maestro" });
      });
  ...
  update_maestro: function (req, res) {
    ...
    Maestros.findOneAndUpdate(
      ...
      (err, maestroUpdate) => {
        ...
        if (!maestroUpdate)
          return res.status(484).json({ message: "No existe el maestro." });
